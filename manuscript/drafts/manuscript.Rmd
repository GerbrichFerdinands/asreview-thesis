...
add that htis is a systematic/rigorous process?-->

Systematic reviews are costly. Conducting a systematic review involves the manual screening of thousands of titles and abstracts, identifying publications relevant to asnwering the research question. This process typically requires over a year of work from a team of researchers [@Borah2017]. Besides time-consuming, the process of title and abstract screening is also prone to human error [@Wang2020].
...
As time and resources are limited, it is often impossible to meet the strict criteria of a systematic review. 
Currently, the demand for systematic reviews exceeds the available capacity by far [@Lau2019].
Moreover, the required resources for a systematic review is making it difficult to timely respond to urgent matters.
An example of this is the need for clinical guidelines in the current covid-19 crisis. 
To ensure a timely review, reducing workload in systematic reviews is imperative. 

With advances in Artificial Intelligence (AI), a plethora of methods have been proposed to reduce workload in systematic reviews [@Harrison2020]. More specifically, the field of AI-aided title and abstract screening is rapidly evolving [@OMara-Eves2015].
Machine Learning (ML) models have been proposed, aiming to assist the reviewer by detecting whether a given publication is relevant or irrelevant to the systematic review. Findings suggest such models potentially reduce workload with 30-70% at the cost of losing 5% of relevant publications (95% recall) [@OMara-Eves2015].
Screening prioritization is a well-established approach in increasing efficiency in title and abstract screening [@Yu2018] [moresources].
The publications to be screened are ordered by their likeliness of relevance, seeing all relevant publicatoins much earlier on in the screening process when they would have been read in a random order. 

Screening prioritization reorders   
Their objective is to assist the reviewer in the screening process by prioritizing publications most likely to be relevant. 

recent research/promising Active Learning. 

The current study focus is on reducing workload by prioritizing the most relevant abstracts. Presenting the abstracts orders the abstracts by likelihood of relevance, e.g. th The reviewer sees the most relevant abstracts first, If the model performs well all relevent abstracts will be seen much earlier than when abstracts are read at a random order.

The algorithm aims to compute which papers in the pool need to be excluded and which need to be included, based on the reviewers decisions. It learns from the reviewers decisions and asks the reviewer to provide more labels, incrementally improving its class predictions.

In most recent studies, 
The latest state-of-the-art methods implement active learning, where the model improves its prediction of relevance based on the reviewers latest decisions in the screening process [@Yu2018; @Miwa2014]. 
The ML-model, where the ML-model orders the most
...
Screening a title + abstracts takes 30 seconds on average, for an experienced reviewer, and longer for inexperienced ones [@Wallace2010].

<!-- add: current consensus active learning are state of the art: how do they woroK; by prioritizatio (put in a picture)  -->

<!-- #### identifying the niche  -->
Although considerable research has been devoted to the effectiveness of ML-models in reducing workload in title and abstract screening, the complex nature of the field makes it difficult to draw overarching conclusions about best practice [@OMara-Eves2015]. The lack of replication on data outside the biomedical sciences makes it impossible to draw conclusions about the general effectiveness of such technologies [@OMara-Eves2015]. 
Taken together, these issues prohibit the adoptation of AI-aided title and abstract screening by systematic reviewers.  
...
The current study aims to address these concerns in the following ways:

1) Combining latest insights from the field of AI-aided title and abstract screening, various ML-models for screening prioritization are built.
2) To evaluate model performance across different review contexts, the models are simulated on multiple datasets from various research areas, of which the data is openly accessible.
3) To ensure replication and transparency, the simulations are implemented in an open source pipeline called `ASReview`. 
<!-- The tool is generic such that any researcher can adjust modules to increase efficiency in this field.   -->

This study set out to increase the evidence base of ML-models aiming to reduce workload in title and abstract screening in systematic reviews. Working towards a general consensus in this emerging field, performance of state-of-the art ML-models is evaluated over various research contexts. To further increase usability and acceptability of such models amongst systematic reviewers, the study presents a reproducible workflow. 
...
The remaining part of this paper is organized as follows: first, a short review of state-of-the art methods is given. Second, the proposed ML-models will be explained on a conceptual level. Third, the methods section gives a detailed description of the study design. The results section ... and last, the results will be discussed. 

--- 

Active learning is the approach where   [@Settles2002]

Study selection in systematic review process
...
##### indicating the structure of the writing)
principle findings:
...
automatically (by machine learning techniques)
In the current study we focus on the effectiveness of different machine learning models in reducing workload in the citation screening process. 
Assisting the reviewer. Combining various machine learning techniques. 
Combining recent insights on, we propose ASReview.
Approaching the citation screening phase as a classfication task,
The benefit is that ASReview is dynamic, user can decide
open source, multiple classifiers and other settings, 

The current study focus is on reducing workload by prioritizing the most relevant abstracts. Presenting the abstracts orders the abstracts by likelihood of relevance, e.g. th
The reviewer sees the most relevant abstracts first, 
If the model performs well all relevent abstracts will be seen much earlier than when abstracts are read at a random order.
