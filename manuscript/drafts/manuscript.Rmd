...
table_nums(name = "datasets", caption = "Descriptive statistics on articles and resulting datasets for each original systematic review.")
```
...
ML tools as well
Literature reviews 
machine learning algorithms have been proposed to reduce workload in abstract screening. 
...
Descriptive statistics on the reviews can be found in `r table_nums("datasets", display = "cite")`. 
...
tibble(Dataset = names(all), 
       ...
       `No. studies` = sapply(all, function(x) x[1,1]), 
       `No. selected for fulltext screening` = sapply(all, function(x) x[1,2]),
       `No. final inclusions` = sapply(all, function(x) x[1,3]),
       `Inclusion rate (%)` = sapply(all, function(x) round(x[1,3]/x[1,1]*100,2)),
       `No. studies in test collection` = sapply(all, function(x) x[3,1]),
       `No. selected for fulltext screening test collection` = sapply(all, function(x) x[3,2]),
       `No. final inclusions in test collection` = sapply(all, function(x) x[3,3]),
       `Inclusion rate in test collection (%)` = sapply(all, function(x) round(x[3,3]/x[3,1]*100,2))
       ) %>%
  # knitr kable
  kable(format = "latex",
        booktabs = TRUE,
        caption = table_nums("datasets")) %>%
  kable_styling(full_width = TRUE) %>%
  add_header_above(c("", "Original study" = 4, "Test collection" = 4))
...
#### ACE
The ACEInhibitors dataset from the study by [@Cohen2006]. a machine learning-based citation classification tool to reduce workload in systematic reviews of drug class efficacy. 
