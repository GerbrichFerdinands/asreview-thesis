...
final <- final[final$title != "NA", ]

# remove duplicates (171 duplicates)
final <- final %>%
  distinct(abstract, .keep_all = TRUE)
table(final$inclusion_code)


nrow(final)
final$abstract_included %>% table()
final$included %>% table()

# 
write.csv(final, "data/nagtegaal.csv")
